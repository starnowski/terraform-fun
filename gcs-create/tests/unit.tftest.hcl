...
run "verify_bucket_creation_with_defaults" {
  ...
  assert {
    condition     = google_storage_bucket.bucket.location == "US"
  ...
  assert {
    condition     = google_storage_bucket.bucket.force_destroy == false
...
run "verify_bucket_creation_with_custom_top_levels" {
  ...
  assert {
    condition     = google_storage_bucket.bucket.name == "test-bucket-custom"
    error_message = "Bucket name did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.location == "EU"
    error_message = "Bucket location did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.project == "my-custom-project"
  ...
  assert {
    condition     = google_storage_bucket.bucket.storage_class == "NEARLINE"
    error_message = "Bucket storage_class did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.rpo == "ASYNC_TURBO"
    error_message = "Bucket rpo did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.enable_object_retention == true
    error_message = "Bucket enable_object_retention did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.labels.env == "prod"
    error_message = "Bucket label 'env' did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.labels.team == "devops"
...
run "verify_bucket_creation_with_nested_blocks" {
  ...
  assert {
    condition     = google_storage_bucket.bucket.autoclass[0].enabled == true
    error_message = "Autoclass enabled did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.autoclass[0].terminal_storage_class == "ARCHIVE"
    error_message = "Autoclass terminal_storage_class did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.cors[0].max_age_seconds == 3600
    error_message = "CORS max_age_seconds did not match"
  }

  assert {
    condition     = contains(google_storage_bucket.bucket.custom_placement_config[0].data_locations, "europe-west1")
    error_message = "Custom placement data_locations did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.encryption[0].default_kms_key_name == "projects/my-project/locations/global/keyRings/my-keyring/cryptoKeys/my-key"
    error_message = "Encryption key name did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.hierarchical_namespace[0].enabled == true
    error_message = "Hierarchical namespace enabled did not match"
  }

  assert {
    condition     = tolist(google_storage_bucket.bucket.lifecycle_rule[0].action)[0].type == "Delete"
    error_message = "Lifecycle action type did not match"
  }

  assert {
    condition     = tolist(google_storage_bucket.bucket.lifecycle_rule[0].condition)[0].age == 30
    error_message = "Lifecycle condition age did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.logging[0].log_bucket == "my-logging-bucket"
    error_message = "Logging log_bucket did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.retention_policy[0].retention_period == "604800"
    error_message = "Retention policy retention_period did not match"
  }

  assert {
    condition     = google_storage_bucket.bucket.soft_delete_policy[0].retention_duration_seconds == 2592000
  ...
  assert {
    condition     = google_storage_bucket.bucket.website[0].main_page_suffix == "index.html"
